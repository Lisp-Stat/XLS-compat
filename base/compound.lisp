...
;;; Here for supporting XLisp-Stat code. Most have equivalents are in
;;; Common Lisp.
...
;;; 
;;; The current mandate for Lisp Stat is to use the internal structure
;;; when possible -- silly to be redundant!  However, this means we
;;; need to understand what sequences as implemented in XLisp (and
;;; XLisp Lisp-Stat) intended to do, which I'm not clear on yet.
...
argument.  Shape refers to the primary approach that we might have
...
;;; WARNING: Using a compound-data class is likely to have bad
;;; consequences.

;;; This class and its methods are only partially defined in
;;; xlisp-stat, and do not appear to be used at all. Instead the
;;; 'built-in' functions, like compound-data-sequence (defined above)
;;; are used rather than the class methods.

(defclass compound-data () ())

;;; TODO: Figure out what arguments these methods should take.
;;; Implemented, but unused, in xlisp-stat
(defgeneric data-length (compound-data)
  (:documentation "Return the length of a compound item's data sequence."))

;;; Implemented, but unused, in xlisp-stat.
(defgeneric data-seq (compound-data)
  (:documentation "Return a compound-data object's data sequence."))

;;; Defined, but not implemented in xlisp-stat
(defgeneric make-data (shape sequence)
  (:documentation "Make SEQUENCE into a compound item of the same form as SHAPE."))

;;; Defined, but not implemented in xlisp-stat
(defgeneric select-data (compound-data)
  (:documentation "Sets or retrieves subset of data. Arguments depend on the object."))

(defun objectp (x)
  "Args: x
Returns T if X is an instance of STANDARD-OBJECT; NIL otherwise."
  (if (subtypep (type-of x) (find-class 'standard-object)) t nil))

(defun kind-of-p (x y)
  "Args: (x y)
Returns T if X and Y are objects and X inherits from Y, NIL otherwise."
  (and (objectp x) (objectp y) (subtypep (type-of x) (type-of y))))

(defun compound-object-p (x) (kind-of-p x 'compound-data))
...
A can be a list or an array. If A is a list and INDICES is a single
number then the appropriate element of A is returned. If A is a list
and INDICES is a list of numbers then the sublist of the corresponding
elements is returned.  If A in an array then the number of INDICES
must match the ARRAY-RANK of A.  If each index is a number then the
appropriate array element is returned.  Otherwise the INDICES must all
be lists of numbers and the corresponding submatrix of A is
